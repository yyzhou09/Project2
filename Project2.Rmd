...
setwd("C:/Users/yzhou/OneDrive - VHB/Desktop/ST 558/Project")
```
...
# Data  
I read data from the folder and then split daily data set into train and test set.
```{r}
...
```
# Summarizations   
Based on the summary table, no missing values. As shown in the histogram for shares, most articles had zero shares. According to the correlation table, mullinearity is not an issue for the data set.
```{r}
...
```
# Models  
This step will create two models. One is a regression tree model and the second one is a boosted tree model. The tuning paramter "cp" for the regression tree model was chosen using the leave one out cross validation. I picked the best tree model using the smallest MAE. The boosted tree model used cross-validation with 10 folds. The final chose model for the boosted tree model was determined using the smallest MAE.   
...
The table below shows the RMSE, R-squared, MAE for these two models' predictions using the test set. Models    
```{r}
